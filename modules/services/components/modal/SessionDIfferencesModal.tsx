...
const SessionDifferencesModal = () => (
  <Dialog.Root>
    ...
    <Dialog.Portal>
      <Dialog.Overlay className="bg-black/80 backdrop-blur-sm data-[state=open]:animate-overlayShow fixed inset-0 z-10" />
      <Dialog.Content
        className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-full max-w-[1000px] translate-x-[-50%] translate-y-[-50%] rounded-[32px] bg-white focus:outline-none z-10"
        style={{
      ...
      >
        <div className="text-center bg-skin-200/40 py-12 px-20 rounded-[32px]">
          <Dialog.Title className="text-purple-400 text-xs tracking-[0.14em] leading-3 font-bold text-center mb-16">
            DIFERENCIA
          </Dialog.Title>
          <Dialog.Description
            className={`${playfairDisplay.variable} font-playfair-display`}
          >
            <p
              className={`${playfairDisplay.variable} font-playfair-display text-gold-600 text-[40px] leading-[43px] tracking-wide mb-9 line-clamp-2 px-20`}
            >
